...
import { get24hCount, getActive, getNewUsersRange, getTotalCount, getTotalUsersRange } from "../service/users";
...
export async function statUsersController(app: FastifyInstance, opt: FastifyPluginOptions) {
    ...
    app.get(
        ...
        async (req: FastifyRequest, reply: FastifyReply) => {
            try {
                const data: JWTPayload | undefined = req.routeConfig.jwtData
                const res: ErrorResponse | SuccessResponse = await getActive({email: data?.email, phone: data?.phone, company_id: data?.company_id})
                reply
                    .code(res.code)
                    .type('application/json; charset=utf-8')
                    .send('body' in res ? {body: res.body} : {error: res.error})
